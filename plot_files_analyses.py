...
window_size = int(1e-6 / avg_period)  # (Tempo de Símbolo BLE = 1us)
window = np.ones(window_size) / window_size

# Cálculo da média móvel
f_smooth = np.convolve(f_inst, window, mode='valid') #np.convolve(f_inst, window, mode='same')
...
# 6. Calcular o Drift Rate (Hz/us)
# Drift Rate é a derivada do drift no tempo
# Delta_f / Delta_t (us)
time_axis_smooth = t_edges[window_size:] # Eixo de tempo após a convolução
...
print(f"--- Análise de Drift BLE (Referência: {f_c/1e9:.2f} GHz) ---")
print(f"Frequência Inicial Medida: {f_c/1e6:.4f} MHz")
print(f"Máximo Drift Medido: {max_drift/1e3:.2f} kHz  (Limite: ±50 kHz)")
print(f"Máximo Drift Rate: {max_drift_rate:.2f} Hz/us (Limite: 400 Hz/us)")

if max_drift <= 50e3 and max_drift_rate <= 400:
    print("\nResultado: PASS (Dentro das especificações BLE)")
else:
...
t_edges = t_edges[:len(f_smooth)]  # Ajusta o tamanho de t_edges para corresponder ao tamanho de fout
f_required = 2.48e9 #np.mean(fout[1])  # Hz
f_req_array = np.full_like(f_smooth, f_c)
...
axs[0, 0].plot(t_edges * 1e6, (f_smooth - f_req_array)/1e3, label=label1, color='red')
...
axs[1, 1].plot(t_edges * 1e6,  (f_smooth / 1e6), label=label1, color='red')
